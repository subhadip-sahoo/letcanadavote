	...
	switch($_POST['do']) {
		...
		case __('Edit Poll', 'wp-polls'):
			...
			$update_latestpoll = update_option('poll_latestpoll', $latest_pollid);
                        $get_id = $wpdb->get_results("SELECT * FROM $wpdb->pollsq WHERE `pollq_id` = {$pollq_id}");
                        if(!empty($get_id)){
                            foreach ($get_id as $id) {
                                $post_id = $id->pollq_post_id;
                            }
                        }
                        $post = get_post( $post_id);
                        $poll_description = $post->post_content;
                        $my_post = array(
                           'ID' => $post_id,
...
switch($mode) {
	...
	case 'edit':
		...
		$poll_totalvoters = intval($poll_question->pollq_totalvoters);
                $get_id = $wpdb->get_results("SELECT * FROM $wpdb->pollsq WHERE `pollq_id` = {$poll_id}");
                if(!empty($get_id)){
                    foreach ($get_id as $id) {
                        $post_id = $id->pollq_post_id;
                    }
