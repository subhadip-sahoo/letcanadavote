	...
	switch($_POST['do']) {
		...
		case __('Add Poll', 'wp-polls'):
			...
			$pollq_question = addslashes(trim($_POST['pollq_question']));                        
                        // Poll Description
			$pollq_description = addslashes(esc_attr(trim($_POST['poll_description'])));
			// Poll Start Date
			$timestamp_sql = '';
			...
			// Insert Poll
			$add_poll_question = $wpdb->query("INSERT INTO $wpdb->pollsq VALUES (0, '$pollq_question', '$pollq_timestamp', 0, $pollq_active, '$pollq_expiry', $pollq_multiple, 0, 0)");
			...
			cron_polls_place();
                        $my_post = array(
                          ...
                          'post_content'  => $pollq_description,
                          'post_status'   => 'publish',
                          'post_author'   => 1,
                          'post_type' => 'polls'
                        );
                        $new_post_id = wp_insert_post( $my_post );
                        $wpdb->update($wpdb->pollsq, array('pollq_post_id' => $new_post_id), array('pollq_id' => $polla_qid));
			break;
	}
}
...
<div class="wrap">
	...
	<table class="form-table">
        ...
        <!-- Poll Description -->
        <h3><?php _e('Poll Description', 'wp-polls'); ?></h3>
        <table class="form-table">
		<tr>
			<th width="20%" scope="row" valign="top"><?php _e('Description', 'wp-polls') ?></th>
			<td width="80%"><?php wp_editor(stripslashes_deep($poll_description), 'poll_description', array('media_buttons' => false));?></td>
		</tr>
	</table>
	<!-- Poll Answers -->
