...
<?php
	global $wpdb;
	$get_polls = $wpdb->get_results("SELECT * FROM $wpdb->pollsq WHERE `pollq_id` = {$poll_id}");
	if(!empty($get_polls)){
		foreach($get_polls as $poll){
			$post_id = $poll->pollq_post_id;
		}
	}
?>
<div class="wrap">
	<h3><?php _e('Comments Count', 'wp-polls'); ?></h3>	
	<div id="poll_logs_display">
		<table class="widefat">
			<tr>
				<td>Total Comments</td>
				<td><?php echo get_comments_number($post_id);?></td>
			</tr>
		</table>
	</div>
</div>
